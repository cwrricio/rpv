...
**Base de verificacao:** requisitos do relatorio final SSQM, `README.md`, `docs/ARCHITECTURE.md`, ADRs em `docs/adr/`, manifests de dependencias, configuracoes Firebase/GitHub e codigo-fonte.

## Conferencia de conformidade do relatorio

| Requisito pedido | Status | Onde esta atendido |
|---|---|---|
| Visao Arquitetural Atual - AS-IS | Atendido | Secao 1, com diagrama textual, componentes e achados arquiteturais. |
| Inventario de Dependencias - tabela completa | Atendido | Secao 2, cobrindo dependencias diretas, operacionais e supply chain; dependencias transitivas estao representadas pelos ecossistemas npm/PyPI. |
| Lock-In Smells Identificados por categoria | Atendido | Secao 3, com evidencias e classificacao por categoria. |
| Resultado do Checklist e pontuacao SSQM | Atendido | Secao 4, com dimensoes, justificativas, total e nivel SSQM. |
| Indices calculados: IDC, IP, IDAN, SSQMScore | Atendido | Secao 5, com criterios, matriz de calculo e interpretacao. |
| Arquitetura Alvo - TO-BE | Atendido | Secao 6, com diagrama textual e decisoes de soberania. |
| Roadmap de Migracao 0-6, 6-18 e 18-36 meses | Atendido | Secao 7, com prioridades, dependencias afetadas e estimativas. |
| Respostas objetivas esperadas | Atendido | Secao 8, respondendo diretamente as seis perguntas finais. |
...
- A documentacao arquitetural mistura estado atual e alvo: `docs/ARCHITECTURE.md` descreve Firestore, Firebase Authentication e Cloud Run Jobs, enquanto o codigo atual ainda usa RTDB e nao possui `functions/jobs/harvest_docentes.py`.
...
| CLS-08 | Documentacao arquitetural parcialmente divergente | Knowledge/Strategic Smell | `docs/ARCHITECTURE.md` descreve Firestore/Auth/Jobs como arquitetura operacional, mas o codigo atual ainda usa RTDB e nao possui `functions/jobs/harvest_docentes.py`; tambem referencia `CONTEXT.md`, ausente no repositorio | Reduz autonomia de novas equipes e dificulta governanca de migracao. | Moderada |
...
Escala de leitura adotada para o SSQMScore:

| Faixa | Nivel | Leitura |
|---|---|---|
| 0% a 20% | Nivel 1 | Dependencia critica |
| 21% a 40% | Nivel 2 | Dependencia alta |
| 41% a 60% | Nivel 3 | Dependencia moderada |
| 61% a 80% | Nivel 4 | Soberania parcial |
| 81% a 100% | Nivel 5 | Soberania alta |
...
- **CLSn - Cloud Lock-In Smells normalizados:** media dos pesos normalizados das severidades dos smells. Pesos usados: Critica `1,00`, Alta `0,80`, Moderada `0,60`, Baixa `0,20`.
...
### Matriz de calculo dos indices

| Dependencia critica avaliada | Risco usado no IDC | Portabilidade usada no IP |
|---|---:|---:|
...
- `IDC = 4,80 / 9 = 0,53`
- `IP = ((26 / 9) / 5) = 0,58`
- `CLSn = (1,00 + 3 * 0,80 + 4 * 0,60 + 1 * 0,20) / 9 = 0,67`
- `SSQMScore = 39 / 90 = 0,43`
- `IDAN = (0,53 + 0,67 + 0,42 + 0,57) / 4 = 0,55`
