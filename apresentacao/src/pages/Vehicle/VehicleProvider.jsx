import { createContext, useContext, useState, useEffect } from "react";

const VehicleContext = createContext(null);
export const useVehicles = () => useContext(VehicleContext);

export default function VehicleProvider({ children }) {
  const [vehicles, setVehicles] = useState([]);
  const [showForm, setShowForm] = useState(false);

  const API = (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000").replace(
    /\/$/,
    ""
  );

  useEffect(() => {
    let mounted = true;
    async function load() {
      try {
        const res = await fetch(`${API}/veiculos`);
        if (!res.ok) return setVehicles([]);
        const json = await res.json();
        const arr = Array.isArray(json)
          ? json
          : Object.keys(json || {})
              .filter((k) => k !== "_")
              .map((k) => ({ id: k, ...json[k] }));
        if (mounted) setVehicles(arr);
      } catch (e) {
        if (mounted) setVehicles([]);
      }
    }
    load();
    return () => (mounted = false);
  }, []);

  async function addVehicle(payload) {
    const res = await fetch(`${API}/veiculos`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    if (!res.ok) throw new Error("Erro ao criar veículo");
    const r2 = await fetch(`${API}/veiculos`);
    const json2 = await r2.json();
    const arr = Array.isArray(json2)
      ? json2
      : Object.keys(json2 || {})
          .filter((k) => k !== "_")
          .map((k) => ({ id: k, ...json2[k] }));
    setVehicles(arr);
  }

  async function updateVehicle(id, patch) {
    const res = await fetch(`${API}/veiculos/${encodeURIComponent(id)}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(patch),
    });
    if (!res.ok) throw new Error("Falha ao atualizar veículo");
    const updated = await res.json();
    setVehicles((v) =>
      v.map((x) => (String(x.id) === String(id) ? updated : x))
    );
    return updated;
  }

  async function deleteVehicle(id) {
    const res = await fetch(`${API}/veiculos/${encodeURIComponent(id)}`, {
      method: "DELETE",
    });
    if (!res.ok) throw new Error("Falha ao apagar veículo");
    setVehicles((v) => v.filter((x) => String(x.id) !== String(id)));
    return true;
  }

  return (
    <VehicleContext.Provider
      value={{
        vehicles,
        addVehicle,
        updateVehicle,
        deleteVehicle,
        showForm,
        setShowForm,
      }}
    >
      {children}
    </VehicleContext.Provider>
  );
}
